// All allowed words for user to input
const allowedWords = [
  "aahed",
  "aalii",
  "aapas",
...
// Event listener that lets user press enter to click the submit button
...
function topText(str) {
  const victory = document.querySelector("#top-text");
...
function guessWord() {
  ...
  if (inputValue.length !== 5) {
    ...
    return;
  } else if (!allowedWords.includes(inputValue)) {
    alert(`${inputValue} is not a valid word`);
    return;
  }

  // Now that the word is validated, fill in the grid
  for (let i = 0; i < 5; i++) {
    gridCells[startIndex + i].textContent = inputValue[i];
  }

  // Track which letters have been matched
  const inputArray = inputValue.split("");
  const randomWordArray = randomWord.split("");
  const matched = Array(5).fill(false); // To track exact matches
  ...
  // Second pass: Check for letter presence without reusing the same letter (Yellow)
  for (let i = 0; i < 5; i++) {
  ...
  // Run if inputted word matches the randomly generated word
  ...
  // Run if all rows are filled without the correct word
  if (inputValue !== randomWord && currentRow === 6) {
    topText(`Game over! The word was ${randomWord.toUpperCase()}`);
